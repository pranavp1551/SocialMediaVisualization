...
public class DLList<E> {
    ...
    /**
     * Sorts the data using the comparator provided with an insertion sort
     * technique
     *
     * @param comparator the comparator that is used for the sort
     */
    public void insertionSort(Comparator<E> comparator) {
        int n = size;

        DLList.Node<E> key = head.next;
        for (int i = 1; i < n; ++i) {
            key = key.next();
            int j = i - 1;

            while (j >= 0 && comparator.compare(getNodeAtIndex(j).data,
                getNodeAtIndex(j + 1).data) > 0) {
                E temp = getNodeAtIndex(j + 1).getData();

                getNodeAtIndex(j + 1).data = getNodeAtIndex(j).getData();
                getNodeAtIndex(j).data = temp;
